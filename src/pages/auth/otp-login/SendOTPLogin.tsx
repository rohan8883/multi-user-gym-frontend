...
export default function SendOTPLogin({ next, data }: Readonly<IProps>) {
  ...
  const onSubmit = async (data: yup.InferType<typeof schema>) => {
    try {
      const response = await postLogin.mutateAsync({
        api: authApi.sendOtpLogin,
        data: data
      });
      if (response?.data?.success) {
        next({ ...data, mobile: data?.mobile, token: response?.data?.token });
    ...
    <>
      <div className="px-6 lg:px-0 mx-auto max-w-sm mb-4">
        ...
        <p className="text-center mt-2 text-muted-foreground text-xs">
          Enter your mobile to send OTP to your mobile number
        </p>
      </div>
      <FormProviders
        methods={methods}
